...
class SampleScanner:
    ...
    def gettext(self, nodelist):
        """Given a list of one or more nodes, recuresively finds all text
    ...
    def handleAuthor(self, node):
        for child in node.childNodes:
            ...
            if child.tagName == 'name':
                ...
                print "Author addiliation:", self.gettext([child])

    def handleAuthorName(self, node):
        surname = self.gettext(node.getElementsByTagName("last"))
        givenname = self.gettext(node.getElementsByTagName("forst"))
